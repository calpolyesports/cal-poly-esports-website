...
export const PUT: RequestHandler = async (event) => {
	...
	if (
		!event.locals.user?.adminFor.includes(newClub) ||
		!event.locals.user?.adminFor.includes(oldEvent?.club)
	) {
		return json(
			{
	...
	const updatedEventData = await db.getEventById(oldEvent._id, event.locals.user?.adminFor);
...
export const DELETE: RequestHandler = async (event) => {
	...
	if (!event.locals.user?.adminFor.includes(oldEvent.club)) {
		return json({
			status: 403,
